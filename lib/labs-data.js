export const labsData = {
  ...
  'ethical-hacking': {
    ...
    variants: [
      {
        scenario: 'During a penetration test, you need to gather information about a target organization before launching any attacks. Use passive reconnaissance and OSINT techniques.',
        vulnerable_code: `# Bad approach: jumping straight to active scanning\n$ nmap -A -T4 target.com\n# This is noisy and will trigger IDS alerts immediately`,
        fix_steps: ['Start with passive OSINT (no direct contact with target).', 'Use tools like WHOIS, theHarvester, and Google Dorking.', 'Map out the attack surface before touching the target.'],
        patched_code: `# PASSIVE RECONNAISSANCE WORKFLOW\n# Step 1: WHOIS lookup for domain ownership\n$ whois target.com\n\n# Step 2: Google Dorking for exposed files\n# site:target.com filetype:pdf\n# site:target.com inurl:admin\n\n# Step 3: theHarvester for emails & subdomains\n$ theHarvester -d target.com -b google,linkedin\n\n# Step 4: Check for breached credentials\n# haveibeenpwned.com (API or manual check)`,
        check_logic: (input) => input.includes('whois') || input.includes('theHarvester') || input.includes('dorking') || input.includes('OSINT'),
        explanation: 'Reconnaissance is the FIRST phase of any penetration test. Passive recon (OSINT) gathers information WITHOUT touching the target—using public databases, search engines, and social media. This is how real ethical hackers map out an organization\'s digital footprint before launching a single packet. Jumping straight to Nmap is like breaking down a front door without checking if the back door is unlocked.'
      },
      {
        scenario: 'You need to discover live hosts, open ports, and running services on a target network during a penetration test. Use Nmap scanning techniques strategically.',
        vulnerable_code: `# Bad approach: a loud, full scan with no stealth\n$ nmap -A -T5 192.168.1.0/24\n# Scans everything aggressively — easily detected by IDS/IPS`,
        fix_steps: ['Start with a stealthy SYN scan (-sS).', 'Use service version detection (-sV) on discovered ports.', 'Run OS detection (-O) only on key targets.'],
        patched_code: `# STRATEGIC SCANNING WORKFLOW\n# Step 1: Discover live hosts (ping sweep)\n$ nmap -sn 192.168.1.0/24\n\n# Step 2: Stealthy SYN scan on live hosts\n$ nmap -sS -T2 192.168.1.10\n\n# Step 3: Version detection on interesting ports\n$ nmap -sV -p 22,80,443,8080 192.168.1.10\n\n# Step 4: OS fingerprinting\n$ sudo nmap -O 192.168.1.10\n\n# Step 5: Run vulnerability scripts\n$ nmap --script vuln -p 80,443 192.168.1.10`,
        check_logic: (input) => input.includes('-sS') || input.includes('-sn') || (input.includes('nmap') && input.includes('-sV')),
        explanation: 'Professional penetration testers don\'t blast networks with aggressive scans. They use a phased approach: first find live hosts (-sn), then stealthily check ports (-sS), then fingerprint services (-sV). The SYN scan is called "half-open" because it never completes the TCP handshake—making it harder for basic firewalls to detect. This methodical approach mimics how real attackers work while staying under the radar.'
      },
      {
        scenario: 'During a security audit, you recovered a password hash from a database dump. Use ethical password cracking techniques to test its strength.',
        vulnerable_code: `# Bad approach: trying to guess passwords manually\n$ echo "Let me try password123... admin... qwerty..."\n# This will take forever and is totally ineffective`,
        fix_steps: ['Identify the hash type (MD5, SHA256, bcrypt, etc.).', 'Use Hashcat or John the Ripper with a wordlist.', 'Apply rules for common password mutations.'],
        patched_code: `# PASSWORD CRACKING WORKFLOW\n# Step 1: Identify the hash type\n$ hashid "5f4dcc3b5aa765d61d8327deb882cf99"\n# Output: MD5\n\n# Step 2: Use John the Ripper with a wordlist\n$ john --format=raw-md5 --wordlist=/usr/share/wordlists/rockyou.txt hashes.txt\n\n# Step 3: Or use Hashcat (GPU-accelerated)\n$ hashcat -m 0 -a 0 hashes.txt rockyou.txt\n# -m 0 = MD5, -a 0 = dictionary attack\n\n# Step 4: Apply mutation rules\n$ hashcat -m 0 -a 0 hashes.txt rockyou.txt -r rules/best64.rule`,
        check_logic: (input) => input.includes('hashcat') || input.includes('john') || input.includes('wordlist'),
        explanation: 'Password cracking is a core ethical hacking skill used to test password policies. Tools like Hashcat use your GPU to test BILLIONS of hashes per second. The "rockyou.txt" wordlist contains 14 million real passwords from a data breach. "Rules" apply mutations like "password" → "P@ssw0rd!" automatically. If your audit cracks passwords easily, the organization needs stronger password policies.'
      },
      {
        scenario: 'You suspect an attacker is hiding malicious data inside innocent-looking image files. Use steganography detection techniques to investigate.',
        vulnerable_code: `# Bad approach: just opening the image normally\n$ open vacation_photo.jpg\n# "Looks like a normal photo to me!"`,
        fix_steps: ['Use steghide or binwalk to detect hidden data.', 'Check file metadata with exiftool.', 'Compare file size to expected dimensions.'],
        patched_code: `# STEGANOGRAPHY DETECTION WORKFLOW\n# Step 1: Check metadata for anomalies\n$ exiftool vacation_photo.jpg\n\n# Step 2: Look for embedded files with binwalk\n$ binwalk vacation_photo.jpg\n\n# Step 3: Extract hidden data with steghide\n$ steghide extract -sf vacation_photo.jpg\n# Enter passphrase (or try empty passphrase)\n\n# Step 4: Check for appended data after EOF\n$ xxd vacation_photo.jpg | tail -20\n\n# Step 5: Use strings to find readable text\n$ strings vacation_photo.jpg | grep -i "password\\|secret\\|flag"`,
        check_logic: (input) => input.includes('steghide') || input.includes('binwalk') || input.includes('exiftool'),
        explanation: 'Steganography is the art of hiding data inside other data—like embedding a secret message or malware inside a normal-looking photo. Tools like steghide can embed AND extract hidden files. binwalk looks for file signatures buried inside other files. Attackers use this to exfiltrate data past security tools that only scan for known malware signatures. A 2MB photo of a sunset could secretly contain an entire stolen database.'
      },
      {
        scenario: 'You are testing whether employees on a public Wi-Fi network are vulnerable to Man-in-the-Middle (MITM) attacks. Demonstrate the attack and the defense.',
        vulnerable_code: `# VULNERABLE: User connects to public Wi-Fi normally\n$ connect-wifi "Free_Coffee_WiFi"\n$ curl http://example.com/login\n# Credentials sent in plaintext over untrusted network!`,
        fix_steps: ['Demonstrate ARP spoofing with arpspoof (authorized test only).', 'Capture traffic with Wireshark to show the risk.', 'Implement the defense: VPN, HTTPS enforcement, HSTS.'],
        patched_code: `# ATTACK DEMONSTRATION (authorized test only!)\n# Step 1: Enable IP forwarding\n$ echo 1 > /proc/sys/net/ipv4/ip_forward\n# Step 2: ARP spoof to intercept traffic\n$ arpspoof -i wlan0 -t 192.168.1.50 192.168.1.1\n\n# DEFENSE (what users should do)\n# 1. Always use a VPN on public networks\n$ openvpn --config my_vpn.ovpn\n# 2. Verify HTTPS on all login pages\n# 3. Enable HSTS on your servers\n# 4. Use DNS-over-HTTPS (DoH) to prevent DNS spoofing`,
        check_logic: (input) => input.includes('arpspoof') || input.includes('VPN') || input.includes('MITM') || input.includes('ip_forward'),
        explanation: 'On public Wi-Fi, an attacker can use ARP spoofing to trick your device into sending ALL traffic through their computer first. They become the "man in the middle" and can read passwords, session tokens, and private messages in real time. The defense is layered: use a VPN to encrypt all traffic, only visit HTTPS sites, and enable HSTS headers on your own servers to prevent SSL stripping attacks.'
      },
      {
        scenario: 'You are performing a web application penetration test. Use Burp Suite to intercept and modify HTTP requests to find vulnerabilities.',
        vulnerable_code: `# Bad approach: testing with just a browser\n$ curl http://target.com/login -d "user=admin&pass=test"\n# No visibility into request/response headers or hidden params`,
        fix_steps: ['Configure Burp Suite as a proxy.', 'Intercept and modify requests in the Proxy tab.', 'Use Repeater to test payloads and Intruder for fuzzing.'],
        patched_code: `# BURP SUITE WORKFLOW\n# Step 1: Set browser proxy to 127.0.0.1:8080\n# Step 2: Intercept login request in Proxy tab\n# Step 3: Send request to Repeater (Ctrl+R)\n# Step 4: Modify parameters and resend:\n#   POST /login HTTP/1.1\n#   user=admin' OR '1'='1&pass=anything\n# Step 5: Analyze the response for SQL errors\n# Step 6: Use Intruder for automated fuzzing:\n#   - Set payload positions on parameters\n#   - Load wordlist of common payloads\n#   - Analyze response lengths for anomalies`,
        check_logic: (input) => input.includes('Burp') || input.includes('Repeater') || input.includes('Intruder') || input.includes('proxy'),
        explanation: 'Burp Suite is the industry-standard tool for web application testing. It acts as a proxy between your browser and the server, letting you see and modify every HTTP request and response. The Repeater lets you manually tweak and resend requests to test for SQLi, XSS, and other flaws. The Intruder automates this by trying thousands of payloads. Almost every professional pen tester uses Burp Suite daily.'
      },
      {
        scenario: 'A web application uses a CAPTCHA to prevent automated attacks. Test whether the CAPTCHA implementation can be bypassed.',
        vulnerable_code: `# Bad approach: manually solving CAPTCHAs one by one\n$ echo "I'll just solve each CAPTCHA by hand..."\n# This doesn't test the actual security of the implementation`,
        fix_steps: ['Check if the CAPTCHA token is validated server-side.', 'Test if the same token can be reused (replay attack).', 'Check if removing the CAPTCHA parameter still allows the request.'],
        patched_code: `# CAPTCHA BYPASS TESTING\n# Test 1: Remove the CAPTCHA field entirely\n$ curl -X POST http://target.com/login \\\n  -d "user=admin&pass=test"\n# If login works without captcha field → BROKEN!\n\n# Test 2: Replay a previously valid token\n$ curl -X POST http://target.com/login \\\n  -d "user=admin&pass=test&captcha=OLD_TOKEN_123"\n# If it accepts an old token → BROKEN!\n\n# Test 3: Send an empty captcha value\n$ curl -X POST http://target.com/login \\\n  -d "user=admin&pass=test&captcha="\n\n# PROPER DEFENSE: Server must validate captcha\n# on EVERY request with a one-time token`,
        check_logic: (input) => input.includes('captcha') || input.includes('CAPTCHA') || input.includes('replay') || input.includes('token'),
        explanation: 'Many CAPTCHA implementations look secure but have fatal flaws. Common bypasses include: removing the CAPTCHA field entirely (server doesn\'t check), replaying old tokens (server doesn\'t invalidate used tokens), or sending empty values (server accepts null). A proper implementation must validate the CAPTCHA server-side on every single request and ensure each token can only be used once.'
      },
      {
        scenario: 'You are testing whether a web application\'s two-factor authentication (2FA) can be bypassed. Identify common implementation flaws.',
        vulnerable_code: `# Flawed 2FA: the app checks 2FA on the login page\n# but doesn't verify it on the actual dashboard endpoint\n$ curl http://target.com/login -d "user=admin&pass=correct"\n# Response: "Please enter your 2FA code"\n$ curl http://target.com/dashboard -b "session=abc123"\n# Response: Welcome, Admin! (2FA BYPASSED!)`,
        fix_steps: ['Check if 2FA can be skipped by navigating directly to authenticated pages.', 'Test if the 2FA code is rate-limited.', 'Verify that the session isn\'t fully authenticated until 2FA completes.'],
        patched_code: `# 2FA BYPASS TESTING\n# Test 1: Direct navigation bypass\n$ curl http://target.com/dashboard -b "session=abc123"\n# Should return 403 if 2FA not completed\n\n# Test 2: Brute-force the 6-digit code\n$ for code in $(seq 000000 999999); do\n    curl -s http://target.com/verify-2fa \\\n      -d "code=$code" -b "session=abc123"\n  done\n# Should be rate-limited after ~5 attempts\n\n# PROPER DEFENSE:\n# - Session must track 2FA_VERIFIED = false\n# - ALL authenticated routes must check 2FA_VERIFIED\n# - Rate-limit 2FA attempts (lockout after 5 tries)\n# - Use time-based codes (TOTP) that expire in 30s`,
        check_logic: (input) => input.includes('2FA') || input.includes('TOTP') || input.includes('rate-limit') || input.includes('brute'),
        explanation: 'Two-factor authentication is only as strong as its implementation. The most common bypass is "direct navigation"—after entering the password, the attacker skips the 2FA page and goes straight to /dashboard. If the server doesn\'t track whether 2FA was actually completed in the session, the attacker gets full access. Other bypasses include brute-forcing 6-digit codes (only 1 million combinations!) if there\'s no rate limiting.'
      },
      {
        scenario: 'You need to analyze a phishing campaign to understand the attacker\'s techniques and protect your organization. Dissect the attack chain.',
        vulnerable_code: `# Bad approach: just deleting the phishing email\n$ echo "Looks suspicious, I'll just delete it"\n# No analysis = no protection against future attacks`,
        fix_steps: ['Analyze email headers to trace the sender.', 'Inspect the phishing URL without clicking it.', 'Identify the type of phishing (spear, whaling, smishing, vishing).'],
        patched_code: `# PHISHING ANALYSIS WORKFLOW\n# Step 1: View raw email headers\n# Check: Return-Path, Received, X-Originating-IP\n# Verify: SPF, DKIM, DMARC results\n\n# Step 2: Analyze the phishing URL safely\n$ curl -sI "http://phish.example.com/login" | head -20\n# Check: redirects, SSL certificate, domain age\n\n# Step 3: Classify the attack type\n# - Phishing: mass, generic "Dear Customer"\n# - Spear Phishing: targeted, uses your real name\n# - Whaling: targets executives/C-suite\n# - Smishing: SMS-based phishing\n# - Vishing: voice call phishing\n\n# Step 4: Extract and report IOCs\n# - Sender IP, domain, URLs, attachment hashes\n# - Submit to VirusTotal and PhishTank`,
        check_logic: (input) => input.includes('SPF') || input.includes('DKIM') || input.includes('spear') || input.includes('phishing') || input.includes('IOC'),
        explanation: 'Deleting a phishing email without analysis is a missed opportunity. By dissecting the attack, you can identify the attacker\'s infrastructure (IPs, domains), their techniques (impersonation, urgency tactics), and their targets. This intelligence helps you block future attacks, train employees, and report the campaign to authorities. Understanding the difference between mass phishing, spear phishing (targeted), and whaling (executive-targeted) helps you assess the sophistication of the threat actor.'
      },
      {
        scenario: 'After gaining initial access during a penetration test, you need to escalate privileges from a regular user to root/admin. Identify common escalation vectors.',
        vulnerable_code: `# Bad approach: running random exploits from the internet\n$ ./random_exploit_from_github.sh\n# Dangerous! Could crash the system or leave backdoors`,
        fix_steps: ['Enumerate SUID binaries and cron jobs.', 'Check for misconfigured sudo permissions.', 'Look for sensitive files with weak permissions.'],
        patched_code: `# PRIVILEGE ESCALATION ENUMERATION\n# Step 1: Check sudo permissions\n$ sudo -l\n# Look for: (ALL) NOPASSWD entries\n\n# Step 2: Find SUID binaries\n$ find / -perm -4000 -type f 2>/dev/null\n# Check GTFOBins for exploitable binaries\n\n# Step 3: Check cron jobs for writable scripts\n$ cat /etc/crontab\n$ ls -la /etc/cron.d/\n\n# Step 4: Look for passwords in config files\n$ grep -r "password" /etc/ --include="*.conf" 2>/dev/null\n\n# Step 5: Check kernel version for exploits\n$ uname -a\n# Compare against exploit-db.com\n\n# Step 6: Use LinPEAS for automated enumeration\n$ ./linpeas.sh`,
        check_logic: (input) => input.includes('sudo -l') || input.includes('SUID') || input.includes('linpeas') || input.includes('cron'),
        explanation: 'Privilege escalation is how a hacker goes from "normal user" to "admin/root." Common vectors include: misconfigured sudo rules (e.g., a user can run vim as root, and vim can spawn a shell), SUID binaries (programs that run as root regardless of who calls them), writable cron jobs (scheduled tasks that run as root), and passwords left in config files. Tools like LinPEAS automate this enumeration. This is why system hardening—removing unnecessary SUID bits, restricting sudo, and securing cron—is so critical.'
      }
    ]
  }
};
